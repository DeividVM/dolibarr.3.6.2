 ...
 */
class pdf_parte_llaves_assista extends ModelePDFProjects
{
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "parte llaves assista";
	...
	function write_file($object, $extrafields, $outputlangs)
	{
		...
		if ($conf->projet->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				$pdf->Cell($total_width, $this->deefault_height, "", 0, 1, 'L', true);
				$style2 = array('width' => 0.6, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(65,65,35));
				$pdf->Line($pdf->GetX()+190/2, $pdf->GetY(), $pdf->GetX()+190, $pdf->GetY(), $style2);
				
				$pdf->SetDrawColor(125,125,125);
				$pdf->SetLineWidth(0.4);
				$pdf->Cell(0, 0, "", 0, 1);
				$pdf->Cell($total_width, $this->deefault_height, "DETALLE DE LAS LLAVES ENTREGADAS", 'B', 1, 'C', true);
				for ($i=0; $i<21; $i++){
					$pdf->Cell($total_width, $this->deefault_height + 1, "", 'BT', 1, 'C', true);
				}
				$pdf->Cell(0, 2, "", 0, 1, 'C');

				$pdf->SetDrawColor(225,225,225);
				$pdf->MultiCell($total_width, 20, "FIRMA, NOMBRE, DNI Y CUÑO DE LA PERSONA QUE RECIBE LAS LLAVES", 1, 'L');
	...
	function _pagehead(&$pdf, $object, $extrafields, $showaddress, $outputlangs)
	{
		...
		$pdf->Cell($total_width, $marge_line, "PARTE DE LLAVES", 0, 1, 'C', true);
		$pdf->Cell($total_width, $marge_line, "", 0, 1, 'C');

		// Logo
		$width_logo = 70;
		
		$logo=$conf->mycompany->dir_output.'/logos/assista_llaves.jpg';
		if ($mysoc->logo)
		{
			if (is_readable($logo))
			{
			    $height=pdf_getHeightForLogo($logo);
			    $pdf->Image($logo, 10, 17, 60, 50);	// width=0 (auto)
			}
			else
			{
        ...
        //print_r($extrafields);
        //echo  '</pre>';
		$pdf->Cell($this->marge_left + $total_width - $width_logo, $this->deefault_height - 1, "DATOS DEL SERVICIO", 0, 2, 'C', true);
		$pdf->Cell($total_width - $width_logo, $this->deefault_height + 1, "INMUEBLE", 0, 2, 'L');
		$pdf->Cell($total_width - $width_logo, $this->deefault_height + 1, "DIRECCIÓN", 0, 2, 'L');
		$pdf->Cell($total_width - $width_logo, $this->deefault_height + 1, "LOCALIDAD", 0, 2, 'L');

		$pdf->Cell(70, $this->deefault_height + 1, "FECHA DE ENTREGA DE LLAVES: ", 0, 0, 'L');
		$pdf->SetFont('', 'U');
		$pdf->Cell(10, $this->deefault_height + 1, "           ", 0, 0, 'C');
		$pdf->SetFont('', '');
		$pdf->Cell(1, $this->deefault_height + 1, "/", 0, 0, 'C');
		$pdf->SetFont('', 'U');
		$pdf->Cell(20, $this->deefault_height + 1, "                      ", 0, 0, 'C');
		$pdf->SetFont('', '');
		$pdf->Cell(1, $this->deefault_height + 1, "/", 0, 0, 'C');
		$pdf->SetFont('', 'U');
		$pdf->Cell(15, $this->deefault_height + 1, "                 ", 0, 1, 'C');
		$pdf->SetFont('', '');

		$pdf->Cell(0, $this->deefault_height + 3, "", 0, 1);
