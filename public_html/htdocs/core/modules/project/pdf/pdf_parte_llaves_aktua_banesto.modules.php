...
class pdf_parte_llaves_aktua_banesto extends ModelePDFProjects
{
	...
	function write_file($object, $extrafields, $outputlangs)
	{
		...
		if ($conf->projet->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				$pdf->SetXY(25, 47);
				$cad = $extrafields_val->ref_K_cli;
				$cad = substr ($cad, 0, strlen($cad) - 1);
				$pdf->Cell(50, 5, $cad, 0, 1, 'L');

				$pdf->SetXY(70, 47);
				$pdf->Cell(50, 5, $extrafields_val->dir_cli, 0, 1, 'L');

				$pdf->SetXY(70, 52);
				$pdf->Cell(50, 5, $extrafields_val->loc_cli, 0, 1, 'L');

				$pdf->SetXY(70, 57);
				$pdf->Cell(50, 5, $extrafields_val->zipcode_cli, 0, 1, 'L');

				$pdf->SetXY(95, 116);
				$pdf->Cell(50, 5, $extrafields_val->pj_api_slct, 0, 1, 'L');
	...
	function _pagehead(&$pdf, $object, $extrafields, $showaddress, $outputlangs)
	{
		...
		$logo=$conf->mycompany->dir_output.'/pdf/kies_aktua_template.jpg';
		if ($mysoc->logo)
		{
			if (is_readable($logo))
			{
