...
// opendoor
$MAIN_DB_PREFIX = 'llx_';
$sql = "SELECT u.rowid, u_ex.prov_oper, u_ex.cat_oper";
$sql.= " FROM ".MAIN_DB_PREFIX."user AS u INNER JOIN ".MAIN_DB_PREFIX."user_extrafields AS u_ex ON u.rowid = u_ex.fk_object";
...
if ($resql)
{
	if ($db->num_rows($resql))
	{
		...
		while ($obj = $db->fetch_object($resql)) {
			$key = $obj->prov_oper.'#'.$obj->cat_oper;
			if (array_key_exists($key, $a)) {
				array_push($a[$key], $obj->rowid);
			} else {
				$a[$key] = array($obj->rowid);
			}
		}
	}
}
...
if (empty($reshook))
{
	...
	if ($action == 'builddoc' && $user->rights->projet->creer)
	{
	    ...
	    $result=project_pdf_create($db, $object, $extrafields, $object->modelpdf, $outputlangs);
	    if ($result <= 0)
	    {
...
if ($action == 'create' && $user->rights->projet->creer)
{
    ...
    {
    	print $object->showOptionals($extrafields,'edit', $json=json_encode($a));
    }
...
//echo "<span class='hidden' id='transferred_values'>".json_encode($a)."</span>";
/*
$(function(){
...
$(document).ready(function() {
		$.transfer = ".json_encode($a).";
		updateOperarioSelect();
		$('ul#ui-id-4').click(function() {
			updateOperarioSelect();
		});
		$('ul#ui-id-7').click(function() {
			updateOperarioSelect();
		});
	})
...
function updateOperarioSelect() {
	var provOper = $('#inputautocompleteoptions_prov_cli').val();
	var id_prov = $('#options_prov_cli option').filter(function() {
	    return this.text == provOper; 
	}).val();

	var catOper = $('#inputautocompleteoptions_cat_oper').val();
	var id_cat = $('#options_cat_oper option').filter(function() {
	    return this.text == catOper; 
	}).val();

	var id_group = (id_prov+'#'+id_cat).trim()

	var re_id_prov = new RegExp('\#[0-9]+$');
	var re_id_cat = new RegExp('[0-9]+\#$');			
	var m;
	
	// ponemos todos los options de los empleados deshabilitados
	$('#options_slct_oper option').each(function() {
		if ($(this).val() != 0){    
			$(this).attr('disabled', true);
		}
	});

	var active_oper = $.transfer[id_group];
	$.each(active_oper, function( index, value ) {
    	$('#options_slct_oper').find('option[value=' + value + ']').attr('disabled', false);
	});

	//$('#inputautocompleteoptions_slct_oper').val('');
	/*
	if ($('#inputautocompleteoptions_cat_oper').val() == ' ') { 
		$.each($.transfer, function(k, v) {
            //display the key and value pair
            if (k.match('^'.concat(id_group.split('#')[0],'\#$'))){ 
            	$.each(v, function( index, value ) {
			    	$('#options_slct_oper').find('option[value=' + value + ']').attr('disabled', false);
				});
            }
        });
	}else if ($('#inputautocompleteoptions_prov_cli').val() == ' ') {
		$.each($.transfer, function(k, v) {
            //display the key and value pair
            if (k.match( '\\\'.concat(id_group,'$'))){
            	$.each(v, function( index, value ) {
			    	$('#options_slct_oper').find('option[value=' + value + ']').attr('disabled', false);
				});
            }
