...
if ($action == 'add' && $canadduser)
{
    ...
    if (! $message)
    {
        ...
        $object->operario       = GETPOST("operario",'alpha');
...
if ($action == 'update' && ! $_POST["cancel"])
{
    ...
    if ($caneditfield)  // Case we can edit all field
    {
       ...
       if (! $error)
       {
            ...
            $object->fk_user    = GETPOST("fk_user")>0?GETPOST("fk_user"):0;
            $object->operario      = GETPOST("operario");
            // Fill array 'array_options' with data from add form
            $ret = $extrafields->setOptionalsFromPost($extralabels,$object);
...
if ($action == 'adduserldap')
{
    ...
    $conf->global->LDAP_FIELD_OPERARIO);
    ...
    if ($result >= 0)
    {
        ...
        if (is_array($ldapusers))
        {
            foreach ($ldapusers as $key => $attribute)
            {
                ...
                $ldap_operario      = $attribute[$conf->global->LDAP_FIELD_OPERARIO];
            }
        }
    }
    else
...
if (($action == 'create') || ($action == 'adduserldap'))
{
    ...
    // Operario
    print $ldap_operario;
    print '<tr><td valign="top">'.$langs->trans("Operario").'</td>';
    print '<td>';
    if (! empty($ldap_operario))
    {
        print '<input id="id_operario" type="hidden" name="operario" value="'.$ldap_operario.'">';
        print $ldap_fax;
    }
    else
    {
        print '<input id="id_operario" type="checkbox" name="operario" value="'.GETPOST('operario').'">';
    }
...
else
{
    ...
    if ($id > 0)
    {
        ...
        if ($action != 'edit')
        {
            ...
            // Operario id_operario_textBoll
            print '<tr><td valign="top">'.$langs->trans("Operario").'</td>';
            print '<script> var status_operario = '.$object->operario.'</script>';
            print '<td id="boolean_operario">'.$object->operario.'</td>';
            print '</tr>'."\n";
        ...
        if ($action == 'edit' && ($canedituser || $caneditfield || $caneditpassword || ($user->id == $object->id)))
        {
            ...
            else
          {
            ...
            print '<tr><td valign="top">'.$langs->trans("Operario").'</td>';
            print '<td>';
            if (! empty($object->operario))
            {
                print '<input id="id_operario" type="checkbox" name="operario" value="'.$object->operario.'">';
            }
            else
            {
                print '<input id="id_operario" type="checkbox" name="operario" value="'.$object->operario.'">';
...
print '<script type="text/javascript" src="'.DOL_URL_ROOT.'/core/js/user_operario_view.js"></script>'."\n";
