...
class User extends CommonObject
{
  ...
  var $skype;
	...
	function fetch($id='', $login='',$sid='',$loadpersonalconf=1)
	{
		...
		// Get user
		$sql = "SELECT u.rowid, u.lastname, u.firstname, u.email, u.job, u.skype, u.signature, u.office_phone, u.office_fax, u.user_mobile, u.operario,";
	...
	function update($user,$notrigger=0,$nosyncmember=0,$nosyncmemberpass=0)
	{
		...
		$this->operario     = trim($this->operario);
		// Check parameters
		if (! empty($conf->global->USER_MAIL_REQUIRED) && ! isValidEMail($this->email))
		{
			$langs->load("errors");
		...
		$sql.= ", operario = '".$this->db->escape($this->operario)."'";
		$sql.= " WHERE rowid = ".$this->id;
	...
	function _load_ldap_info()
	{
        ...
        if ($this->operario && ! empty($conf->global->LDAP_FIELD_OPERARIO))          $info[$conf->global->LDAP_FIELD_OPERARIO] = $this->operario;
	...
	function update_ldap2dolibarr(&$ldapuser)
	{
		...
		$this->operario=$ldapuser->{$conf->global->LDAP_FIELD_OPERARIO};
	...
	function get_full_tree($deleteafterid=0)
	{
		...
		$sql = "SELECT DISTINCT u.rowid, u.firstname, u.lastname, u.fk_user, u.login, u.statut, u.entity, u.operario";	// Distinct reduce pb with old tables with duplicates
		$sql.= " FROM ".MAIN_DB_PREFIX."user as u";
		...
		if ($resql)
		{
			...
			while ($obj = $this->db->fetch_object($resql))
			{
				...
				$this->users[$obj->rowid]['operario'] = $obj->operario;
				$i++;
			}
		}
		else
